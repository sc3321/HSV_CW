// Dafny program tasks_2025.dfy compiled into C#
...
method sort_pair(A: array<int>, i: int, j: int)
  ...
  modifies A
  ensures A[i] <= A[j]
  ensures forall k: int {:trigger old(A[k])} {:trigger A[k]} :: 0 <= k < A.Length && k != i && k != j ==> A[k] == old(A[k])
  decreases A, i, j
{
  if A[j] < A[i] {
    A[i], A[j] := A[j], A[i];
...
namespace _module {
  ...
  public partial class __default {
    ...
    {
      if (((A)[(int)(j)]) < ((A)[(int)(i)])) {
        BigInteger _rhs0 = (A)[(int)(j)];
        BigInteger _rhs1 = (A)[(int)(i)];
        BigInteger[] _lhs0 = A;
        BigInteger _lhs1 = i;
        BigInteger[] _lhs2 = A;
        BigInteger _lhs3 = j;
